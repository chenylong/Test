...
if (typeof jQuery != 'undefined') { 
	...
	$(document).ready( function()
	{
		...
		if(navigator.userAgent.search(/MSIE/)>=0)
		{
			...
			{
			   window.__orphanarticle = location.href.replace(/([cC])ontents\//i, '$1ontents/index.html?');
			}
		}
		else 
		{
...
if(!window.__doNotPutInCookie)
{
	...
	document.cookie = "pcxtmenu="+ contentPagePath;
	
}

if(!window.__doNotPutInCookie && isLocalStorageAvailable() )
{
	localStorage.removeItem("pcxt__menu");
	localStorage.setItem("pcxt__menu", contentPagePath);
}	
